...
interface PlanSelectStepProps {
  ...
  /** モニター確認画面で「応募する」を選んだか。案内バナーの表示だけに使う。 */
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 md:p-8">
        ...
        {/* モニター確認画面で応募済みの場合の案内。特典の対象にはProプランが必要なため、
            プラン選択がいきなり降ってきた印象を和らげる一言を添える。 */}
        {monitorConsent && (
          <div className="flex items-center gap-3 bg-primary-50 border border-primary-100 rounded-xl px-4 py-3 mb-6">
            <Gift className="w-5 h-5 text-primary-600 shrink-0" />
            <p className="text-sm text-primary-700">
              モニター特典へのご応募ありがとうございます。特典の対象となるProプランをご案内しています。
            </p>
          </div>
        )}
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {/* Free Plan。モニター特典に応募済みの場合は選択させない
              （特典の対象はProプランのみのため）。 */}
          <div
            onClick={() => !monitorConsent && onSelectedPlanChange('free')}
            className={`relative p-8 rounded-3xl transition-all flex flex-col ${
              monitorConsent
                ? 'border border-slate-200 bg-slate-50 opacity-60 cursor-not-allowed'
                : selectedPlan === 'free'
                ? 'border-2 border-primary-500 bg-white shadow-xl cursor-pointer'
                : 'border border-slate-200 bg-white shadow-sm hover:shadow-md cursor-pointer'
            }`}
          >
            {!monitorConsent && selectedPlan === 'free' && (
            ...
            {monitorConsent && (
              <p className="text-xs text-slate-500 mb-2">モニター特典はProプラン限定です</p>
            )}
